 ...
 * segmentation violation.  It is 8bit clean. *
 * 
 * Written by Rich $alz, mirror!rs, Wed Nov 26 19:03:17 EST 1986. *  Rich $alz is
 * now <rsalz@bbn.com>. *  April, 1991:  Replaced mutually-recursive calls with
 * in-line code *  for the star character. *
 * 
 * Special thanks to Lars Mathiesen <thorinn@diku.dk> for the ABORT code. * This
 * can greatly speed up failing wildcard patterns.  For example: * ern:
 * -*-*-*-*-*-*-12-*-*-*-m-*-*-* *		text 1:
 * ormal--12-120-75-75-m-70-iso8859-1 *		text 2:
 * ormal--12-120-75-75-X-70-iso8859-1 *  Text 1 matches with 51 calls, while
 * text 2 fails with 54 calls.  Without *  the ABORT code, it takes 22310 calls
 * to fail.  Ugh.  The following *  explanation is from Lars: *  The
 * precondition that must be fulfilled is that DoMatch will consume *  at least
 * one character in text.  This is true if *p is neither '*' nor *  '\0'.)  The
 * last return has ABORT instead of FALSE to avoid quadratic *  behaviour in
